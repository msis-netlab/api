#!/bin/bash
# Brings a pod online in NETLAB

# Initial values, some may be replaced depending on parameters
CURL=`which curl`
DEV_ID=""	# Not sure what these two parameters are for but they are present in the request to bring a pod online so I am including them
PC_ID="0"
...
--data "State=pod_online&pod_id=${POD_ID}&dev_id=${DEV_ID}&pc_id=${PC_ID}" \
--compressed > /dev/null
